...
void copyPeriodic(float *p, float *u, float *v, float *w,
  ...
  // I was right! But it doesn't do much
  // It actually slows it down here.
  for(int i=0;i<ni;++i) {
    int offset = kstart+i*iskip;
    for(int j=0;j<nj;++j) {
...
void computeResidual(float *presid, float *uresid, float *vresid, float *wresid,
  ...
  //if(k%8 == 0 or 1, then the wouldn't the boundary be at the beginning?)
...
int main(int ac, char *av[]) {
  ...
  setInitialConditions(&p[0], &u[0], &v[0], &w[0],
		       ni, nj, nk, kstart, iskip, jskip, L, myId) ;
