...
//If this is the initial condition, is this the originating node necessary for all other nodes?
...
//Is this the 64*64*64? Should I push the MPI Calls here?
...
void copyPeriodic(float *p, float *u, float *v, float *w,
		  int ni, int nj, int nk , int kst#pragma omp atomicart, int iskip, int jskip) {

  const int kskip=1 ;

  // copy the i periodic faces
  // This seems to be an excellent candidate to apply Openmp
  ...
  // I bet I could rewrite this so that it can be collapsed
  //But for now, just use parallel for
  #pragma omp parallel for
  for(int i=0;i<ni;++i) {
    omp_set_max_active_levels(2);
    int offset = kstart+i*iskip;
...
void zeroResidual(float *presid, float *uresid, float *vresid, float *wresid,
  ...
  const int kskip=1 ;
  //Does the collapse effect the max active levels needed
  omp_set_max_active_levels(2);
  #pragma omp parallel for collapse(2)
  for(int i=-1;i<ni+1;++i) {
    for(int j=-1;j<nj+1;++j) {
      int offset = kstart+i*iskip+j*jskip;
      //Can this be moved too? Could collapse be set to 3?
      #pragma omp parallel for 
...
int main(int ac, char *av[]) {
  ...
  //potentially convert this to a for loop and use parallel for
  while(ac >= 2 && av[1][0] == '-') {
